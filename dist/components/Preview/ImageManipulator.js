...
var ImageManipulator = /*#__PURE__*/function (_Component) {
  ...
  function ImageManipulator() {
    ...
    _defineProperty(_assertThisInitialized(_this), "saveImage", function () {
      ...
      var imageMeta = filerobot.imageMeta,
          imageProperties = filerobot.imageProperties,
          imageName = filerobot.imageName,
          _filerobot$saveMode = filerobot.saveMode,
          saveMode = _filerobot$saveMode === void 0 ? _config.SAVE_MODES.DUPLICATE : _filerobot$saveMode;
      ...
      if (!processWithCloudService) {
        ...
        var loweredSaveModeStr = saveMode.toLowerCase();
        var name = imageName || imageNameFromUrl;

        if (loweredSaveModeStr !== _config.SAVE_MODES.REPLACE) {
          var splittedName = name.replace(/-version-.{6}/g, '').split('.');
        ...
        if (loweredSaveModeStr !== _config.SAVE_MODES.NEW) {
      ...
      updateState({
        ...
        if (initialZoom !== 1) {
          ...
          _this.CamanInstanceZoomed.render(function () {
            _this.getCanvas().className = oldCanvas.className;
            updateState({
              isHideCanvas: false,
              isShowSpinner: false
            });
    ...
    _defineProperty(_assertThisInitialized(_this), "applyOrientation", function () {
      ...
      updateState({
        ...
        var oldCanvas = _this.getCanvas(null, true);
        ...
        if (initialZoom !== 1) {
          ...
          _this.CamanInstanceOriginal.render(function () {
            _this.getCanvas(null, true).className = oldCanvas.className;
            updateState({
              rotate: 0,
              flipX: false,
              flipY: false,
    ...
    _defineProperty(_assertThisInitialized(_this), "getWatermarkArguments", function () {
      ...
      var xPos = (0, _utils.roundDecimalPoint)(x / canvasWidth * 100);
      var yPos = (0, _utils.roundDecimalPoint)(y / canvasHeight * 100);
      var biggestScaleRatio = Math.max(watermark.width / canvasWidth, watermark.height / canvasHeight);
      var watermarkScale = (0, _utils.roundDecimalPoint)(biggestScaleRatio * 100);
